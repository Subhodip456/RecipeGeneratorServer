...
const express = require('express');
const nodemailer = require('nodemailer');
const jwt = require('jsonwebtoken'); // ← replaces crypto + in-memory tokens
...
app.post('/send-magic-link', (req, res) => {
  ...
  // JWT is self-contained — no in-memory storage needed, survives serverless
...
// ─── Called from email click → validates JWT, opens app via deep link ─────────
app.get('/verify', (req, res) => {
  const { token } = req.query;
  ...
  res.send(`
    ...
    <html>
      ...
      <body style="font-family:sans-serif;text-align:center;padding-top:50px;">
        ...
        <a href="${deepLink}" style="font-size:20px;color:blue;">Open App</a>
      </body>
    </html>
  `);
});

// ─── Called from mobile app → returns JSON ────────────────────────────────────
